...
if __name__ == '__main__':
    ...
    if args.parallel is None or remote.ray is None:
        ...
        for i in range(args.parallel):
            ...
            # Randomize ball location and velocity
            random_ball_location_x = random.uniform(-1.0, 1.0)
            random_ball_location_y = random.uniform(-1.0, 1.0)
            random_ball_location = [random_ball_location_x, random_ball_location_y]
            random_ball_velocity = [random.uniform(-0.5, 0.5), random.uniform(-0.5, 0.5)]

            result = match.run.remote(team1, team2, args.num_players, args.num_frames, max_score=args.max_score,
                                      initial_ball_location= random_ball_location,
                                      initial_ball_velocity= random_ball_velocity,
                                      record_fn=recorder)
            results.append(result)

        # Add round counter and print data
        round_counter = 0
        for result in results:
            ...
            print('Round', round_counter, 'Match results', result)
            round_counter += 1
