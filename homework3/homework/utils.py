...
class SuperTuxDataset(Dataset):
        ...
        with open(path.join(dataset_path, 'labels.csv'), newline='') as f:
            ...
            for fname, label, _ in reader:
                if label in LABEL_NAMES:
                    image_path = path.join(dataset_path, fname)
    ...
    def __getitem__(self, idx):
        ...
        image_path, label_id = self.data[idx]
        image = Image.open(image_path)
        image.load()
        return to_tensor(image), label_id
...
def load_dense_data(dataset_path, num_workers=0, batch_size=256, **kwargs):
